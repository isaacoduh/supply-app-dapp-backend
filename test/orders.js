...
contract('Orders', (accounts) => {
  ...
  describe('Deliver', () => {
    ...
    it('7. Should not be delivered as there is no order', () => {
      return Orders.deployed().then((instance) => {
    ...
    it('8. Should be delivered', async() => {
      return Orders.deployed().then(async(instance) => {
        ...
        assert.equal(response.logs[0].event, 'OrderDelivered', "Event name does not match OrderDelivered");
      })
    });
  ...
  describe('Refund', () => {
    ...
    it('10. Should not be refuned as there is no order', () => {
      return Orders.deployed().then((instance) => {
        return instance.deliver(orderId);
      }).then(assert.fail).catch((error) => {
        assert(error.message.indexOf("Order is not found") >= 0);
    ...
    it('11. Should not be refunded as the order has not been paid', () => {
      return Orders.deployed().then(async (instance) => {
        ...
        assert(error.message.indexOf("The order has not been paid for") >= 0);
      });
    });
    ...
    it('14. Should be refunded', async() => {
      ...
      assert.equal(refundTxt.logs[0].event, 'OrderRefunded', "Event name does not match OrderRefunded");
      ...
      assert.equal(currentBalance.toString(), firstBalance.toString(), "The current account balance does not match first balance");

      let order = await instance.get(orderId);

      assert.equal(order.status, 3, "Order status does not match Refunded");
    });
